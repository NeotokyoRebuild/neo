...
ConVar neo_round_timelimit("neo_round_timelimit", "3.25", FCVAR_REPLICATED, "Neo round timelimit, in minutes.",
		...
		for (int i = 1; i <= gpGlobals->maxClients; ++i)
		{
			...
			if (pNeoPlayer)
			{
				...
				if (!bClantagSet[iTeam])
				{
					...
					V_strcpy_safe(szTeamClantags[iTeam], pNeoPlayer->m_szNeoClantag.Get()),
					++iHasClantags[iTeam];
				}
				else
				{
					iHasClantags[iTeam] += (V_strcmp(szTeamClantags[iTeam], pNeoPlayer->m_szNeoClantag.Get()) == 0);
				}
			}
		}
...
void CNEORules::ClientSettingsChanged(CBasePlayer *pPlayer)
{
	...
	const char *pszOldNeoClantag = pNEOPlayer->m_szNeoClantag.Get();
