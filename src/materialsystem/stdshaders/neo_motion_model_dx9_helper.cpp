...
#include "neo_motion_model_vs20.inc"
#include "neo_motion_model_ps20b.inc"
...
//-----------------------------------------------------------------------------
void InitParamsNeoMotionModel_DX9( CBaseVSShader *pShader, IMaterialVar** params, const char *pMaterialName, NeoMotionModel_DX9_Vars_t &info )
{	
...
// Initialize shader
//-----------------------------------------------------------------------------
void InitNeoMotionModel_DX9( CBaseVSShader *pShader, IMaterialVar** params, NeoMotionModel_DX9_Vars_t &info )
{
	Assert( info.m_nFlashlightTexture >= 0 );
...
class CNeoMotionModel_DX9_Context : public CBasePerMaterialContextData
{
public:
...
// Draws the shader
//-----------------------------------------------------------------------------
void DrawNeoMotionModel_DX9_Internal( CBaseVSShader *pShader, IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow,
	bool bHasFlashlight, NeoMotionModel_DX9_Vars_t &info, VertexCompressionType_t vertexCompression,
							CBasePerMaterialContextData **pContextDataPtr )
{
	...
	CNeoMotionModel_DX9_Context *pContextData = reinterpret_cast< CNeoMotionModel_DX9_Context *> ( *pContextDataPtr );
	if ( !pContextData )
	{
		pContextData = new CNeoMotionModel_DX9_Context;
	...
	if( pShader->IsSnapshotting() )
	{
		...
		DECLARE_STATIC_VERTEX_SHADER( neo_motion_model_vs20 );
		SET_STATIC_VERTEX_SHADER( neo_motion_model_vs20 );

		// Assume we're only going to get in here if we support 2b
		DECLARE_STATIC_PIXEL_SHADER( neo_motion_model_ps20b );
		SET_STATIC_PIXEL_SHADER_COMBO( FLASHLIGHT, bHasFlashlight );
		SET_STATIC_PIXEL_SHADER_COMBO( FLASHLIGHTDEPTHFILTERMODE, nShadowFilterMode );
		SET_STATIC_PIXEL_SHADER_COMBO( CONVERT_TO_SRGB, 0 );
		SET_STATIC_PIXEL_SHADER( neo_motion_model_ps20b );
	...
	else // not snapshotting -- begin dynamic state
	{
		...
		DECLARE_DYNAMIC_VERTEX_SHADER( neo_motion_model_vs20 );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( DOWATERFOG, fogIndex );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, numBones > 0 );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( LIGHTING_PREVIEW, pShaderAPI->GetIntRenderingParameter(INT_RENDERPARM_ENABLE_FIXED_LIGHTING)!=0);
		SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
		SET_DYNAMIC_VERTEX_SHADER_COMBO( NUM_LIGHTS, lightState.m_nNumLights );
		SET_DYNAMIC_VERTEX_SHADER( neo_motion_model_vs20 );

		DECLARE_DYNAMIC_PIXEL_SHADER( neo_motion_model_ps20b );
		SET_DYNAMIC_PIXEL_SHADER_COMBO( NUM_LIGHTS, lightState.m_nNumLights );
		SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITEWATERFOGTODESTALPHA, bWriteWaterFogToAlpha );
		SET_DYNAMIC_PIXEL_SHADER_COMBO( WRITE_DEPTH_TO_DESTALPHA, bWriteDepthToAlpha );
		SET_DYNAMIC_PIXEL_SHADER_COMBO( PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo() );
		SET_DYNAMIC_PIXEL_SHADER_COMBO( FLASHLIGHTSHADOWS, bFlashlightShadows );
		SET_DYNAMIC_PIXEL_SHADER( neo_motion_model_ps20b );
...
// Draws the shader
//-----------------------------------------------------------------------------
void DrawNeoMotionModel_DX9( CBaseVSShader *pShader, IMaterialVar** params, IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow,
				   NeoMotionModel_DX9_Vars_t &info, VertexCompressionType_t vertexCompression, CBasePerMaterialContextData **pContextDataPtr )

{
	bool bHasFlashlight = pShader->UsingFlashlight( params );
	if ( bHasFlashlight )
	{
		DrawNeoMotionModel_DX9_Internal( pShader, params, pShaderAPI,	pShaderShadow, false, info, vertexCompression, pContextDataPtr++ );
		if ( pShaderShadow )
		{
			pShader->SetInitialShadowState( );
		}
	}
	DrawNeoMotionModel_DX9_Internal( pShader, params, pShaderAPI,	pShaderShadow, bHasFlashlight, info, vertexCompression, pContextDataPtr );
}
