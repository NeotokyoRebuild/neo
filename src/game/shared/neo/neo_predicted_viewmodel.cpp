	...
	const Vector& eyePosition, const QAngle& eyeAngles)
{
	...
	if (auto neoPlayer = static_cast<CNEO_Player*>(pOwner))
	{
		...
		constexpr int VIEWMODEL_MOVE_DISTANCE = 24; // max distance viewmodel can move is actually (1 - 0.375) * VIEWMODEL_MOVE_DISTANCE
		if (m_flGunPushLastChangeTime < gpGlobals->curtime) // Why is this value sometimes greater than current time? Also this fixes gun moving back much slower around some corners, maybe weird prediction stuff?
		{
			...
			UTIL_TraceHull(startPos, endPos, Vector(-4,-4,-4), Vector(4,4,4), MASK_PLAYERSOLID &~ CONTENTS_MONSTER, pOwner, COLLISION_GROUP_NONE, &tr);
			tr.fraction = max(0.375, tr.fraction); // The smallest value tr.fraction can have given the size of the player hull
