...
// NEO TODO (Rain): NEO specific game modes var (CTG/TDM/...)
	...
	SendPropInt(SENDINFO(m_iForcedTeam), NumBitsForRange(-1, TEAM__TOTAL)),
	SendPropInt(SENDINFO(m_iForcedClass), NumBitsForRange(-1, NEO_CLASS_ENUM_COUNT)),
	SendPropInt(SENDINFO(m_iForcedSkin), NumBitsForRange(-1, NEO_SKIN_ENUM_COUNT)),
	...
	SendPropInt(SENDINFO(m_iGhosterPlayer), NumBitsForCount(MAX_PLAYERS), SPROP_UNSIGNED),
	...
	SendPropInt(SENDINFO(m_iJuggernautPlayerIndex), NumBitsForCount(MAX_PLAYERS), SPROP_UNSIGNED),
