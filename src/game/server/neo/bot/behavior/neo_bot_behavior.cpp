...
// 		}
	...
	const QueryResultType qDirectShouldAimQuery = me->GetIntentionInterface()->ShouldAim(me);
	QueryResultType qShouldAimQuery = qDirectShouldAimQuery;
	if (qShouldAimQuery == ANSWER_NO)
	{
		// Delay aiming out to prevent/reduce rapid aiming in-outs
...
QueryResultType CNEOBotMainAction::ShouldAim(const CNEOBot *me) const
{
