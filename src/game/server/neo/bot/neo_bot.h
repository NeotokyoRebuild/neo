...
#define LOOK_FOR_ENEMIES true
	...
	bool IsLineOfFirePenetrationClear(const trace_t &tr, const Vector &from, const Vector &to) const;

	bool IsLineOfSightClear(CBaseEntity *entity, LineOfSightCheckType checkType = IGNORE_NOTHING) const override;
	bool IsLineOfSightClear(const Vector &pos, LineOfSightCheckType checkType = IGNORE_NOTHING, CBaseEntity *entityToIgnore = NULL) const override;
