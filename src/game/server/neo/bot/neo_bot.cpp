...
bool CNEOBot::IsLineOfFirePenetrationClear(const trace_t &tr, const Vector &from, const Vector &to) const
{
	static constexpr const float FL_MAX_METERS_TRY_PENETRATE = 12.0f;
	...
	if (material != 'J')
	{
		...
		if (flMeters <= FL_MAX_METERS_TRY_PENETRATE)
		{
			material -= 'A';
...
// Had to override this function otherwise it'll not find it within CNEOBot
bool CNEOBot::IsLineOfSightClear(const Vector &pos, LineOfSightCheckType checkType, CBaseEntity *entityToIgnore) const
{
	return BaseClass::IsLineOfSightClear(pos, checkType, entityToIgnore);
}
...
bool CNEOBot::IsLineOfFireClear(const Vector& from, const Vector& to, const LineOfFireFlags flags) const
{
	...
	{
		return IsLineOfFirePenetrationClear(trace, from, to);
	}
...
bool CNEOBot::IsLineOfFireClear(const Vector& from, CBaseEntity* who, const LineOfFireFlags flags) const
{
	...
	if (!bIsClear && !(flags & LINE_OF_FIRE_FLAGS_SHOTGUN) && (flags & LINE_OF_FIRE_FLAGS_PENETRATION))
	{
		return IsLineOfFirePenetrationClear(trace, from, who->WorldSpaceCenter());
	}
