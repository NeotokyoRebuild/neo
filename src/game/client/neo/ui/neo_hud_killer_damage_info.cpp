...
void CNEOHud_KillerDamageInfo::UpdateStateForNeoHudElementDraw()
{
	...
	const wchar_t *pwszToggleTypeLabel = L"match";

	switch (cl_neo_kdinfo_toggletype.GetInt())
	{
	case KDMGINFO_TOGGLETYPE_ROUND:
		m_bShowInfo = true;
		pwszToggleTypeLabel = L"round";
		break;
	case KDMGINFO_TOGGLETYPE_NEVER:
		m_bShowInfo = false;
		pwszToggleTypeLabel = L"round";
		break;
	case KDMGINFO_TOGGLETYPE_MATCH:
	default:
		break;
	}
	...
	// Set keybind message
	{
		...
		V_swprintf_safe(m_wszBindBtnMsg, L"Use [%ls] to toggle (%ls) | Previous: [%ls] | Next: [%ls]",
				wszBindToggle, pwszToggleTypeLabel, wszBindPrev, wszBindNext);
	}
		...
		// General infos - Inflicted damage, name, received damage
		{
			...
			for (int i = iCurPageStart; i < iCurPageEnd; ++i)
			{
				...
				// Middle - Attacker's name and class
				{
					...
					// May or may not get both scenario, but killer player always have
					// higher textual precedence than killed by local player(s)
					const wchar_t *pwszExtraInfo = L"";
					if (bKilledYou)
					{
						pwszExtraInfo = L" - Killed you";
					}
					else if (bYouKilled)
					{
						pwszExtraInfo = L" - Killed by you";
					}
					V_swprintf_safe(wszLabel, L"%ls (%ls)%ls", GetNeoClassNameW(m_iClassIdxsList[i]), SZWSZ_NEO_TEAM_STRS[iDmgerTeam].wszStr, pwszExtraInfo);
					NeoUI::Label(wszLabel);
				}
