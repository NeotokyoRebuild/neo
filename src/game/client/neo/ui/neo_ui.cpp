...
void EndContext()
{
	if (c->iRightClick >= 0 && c->iRightClickSection >= 0 && c->pwszRightClickList && c->ipwszRightClickListSize > 0)
	{
		...
		{
			const Dim &dim = c->dimRightClick;
			if (IN_BETWEEN_EQ(dim.x, c->iMouseAbsX, dim.x + dim.wide) &&
...
void TextEdit(const wchar_t *wszLeftLabel, wchar_t *wszText, const int iMaxBytes, const ETextEditFlags flags)
{
	MultiWidgetHighlighter(2);
	Label(wszLeftLabel);
	TextEdit(wszText, iMaxBytes, flags);
}

void TextEdit(wchar_t *wszText, const int iMaxBytes, const ETextEditFlags flags)
{
	BeginWidget();
	...
	if (IN_BETWEEN_AR(0, c->irWidgetLayoutY, c->dPanel.tall))
	{
		...
		case MODE_KEYPRESSED:
		{
			if ((vgui::input()->IsKeyDown(KEY_LCONTROL) || vgui::input()->IsKeyDown(KEY_RCONTROL)) && bHasTextSel)
			{
				switch (c->eCode)
				{
				break; case KEY_X: iCopyAction = COPYMENU_CUT;
				break; case KEY_C: iCopyAction = COPYMENU_COPY;
				break; case KEY_V: iCopyAction = COPYMENU_PASTE;
				break; default: break;
				}
			}
			else if (wdgState.bActive)
			{
					...
					if (iWszTextSize > 0)
					{
						...
						else if (bIsCursor)
						{
							if (iTextSelMin > 0 && bIsLeftDelete)
							{
								...
								V_wcsncpy(wszText, wszStaticTmpText, sizeof(wchar_t) * iMaxBytes);
								--c->iTextSelStart;
							}
		...
		case COPYMENU_COPY:
		{
			if (iTextSelMin < iTextSelMax)
			{
				...
				{
					wszText[iTextSelMin] = L'\0';
					V_snwprintf(wszText, iMaxBytes, L"%ls%ls", wszText, wszText + iTextSelMax);
					bRebuildIrTextWidths = true;
					c->iTextSelStart = iTextSelMin;
		...
		case COPYMENU_PASTE:
		{
			if (vgui::system()->GetClipboardTextCount() > 0)
			{
				...
				if (iClipboardWSZBytes > 0)
				{
					...
					if (bFromEnd)
					{
						V_wcsncat(wszText, wszClipboard, iMaxBytes);
