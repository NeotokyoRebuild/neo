...
void CNEOHud_SpectatorOverlay::UpdateStateForNeoHudElementDraw()
{
	...
	for (const auto iPutTeam : {TEAM_JINRAI, TEAM_NSF})
	{
		for (int i = 1; i <= gpGlobals->maxClients; ++i)
		{
			...
			if (iTeam != iPutTeam || (m_iCardsSize >= MAX_PLAYERS_ARRAY_SAFE))
			{
				continue;
			}
			...
			if (g_pNeoScoreBoard && g_pNeoScoreBoard->m_pImageList && CNEOScoreBoard::ShowAvatars())
			{
				...
				if (steamId.IsValid())
				{
					const CUtlMap<CSteamID, MapAvatarValue> &mapAvatars = g_pNeoScoreBoard->m_mapAvatarsToImageList;
					...
					if (iMapIdx != mapAvatars.InvalidIndex())
					{
						...
						if ((!pCard->pAvatar || !pCard->pAvatar->IsValid()) && avatar.i32Idx >= 0)
						{
							pCard->pAvatar = static_cast<CAvatarImage *>(g_pNeoScoreBoard->m_pImageList->GetImage(avatar.i32Idx));
						}
					}
		...
		const Color accentColor, const float flWideAs43) const
{
	...
	{
		card.pAvatar->m_bDeadAvatar = !card.bAlive;
		card.pAvatar->SetPos(iAvatarX, iAvatarY);
		card.pAvatar->SetSize(iAvatarSize, iAvatarSize);
		card.pAvatar->Paint();
	}
