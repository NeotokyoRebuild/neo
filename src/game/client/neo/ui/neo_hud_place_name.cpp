...
static const char* TEXT_MATERIAL = "editor/worldtext_9";
	...
	: CHudElement(pElementName), Panel(parent, pElementName)
{
	...
	const PointWorldText foo = { "Important Place", {100, 100, 400}};
...
static bool bShouldDrawPlaceNames = false;
static ConCommand startshowplacenames("+showplacenames", [](const CCommand& args)->void {bShouldDrawPlaceNames = true; });
static ConCommand endshowplacenames("-showplacenames", [](const CCommand& args)->void {bShouldDrawPlaceNames = false; });
...
static float flPlaceNameOpacity = 0.f;
void CNEOHud_PlaceName::DrawPlaceNames()
{
	constexpr int ANIMATION_SPEED = 2;
	if (bShouldDrawPlaceNames)
	{
		flPlaceNameOpacity = min(1.0f, flPlaceNameOpacity + (gpGlobals->frametime * ANIMATION_SPEED));
	}
	else
	{
		flPlaceNameOpacity = max(0.0f, flPlaceNameOpacity - (gpGlobals->frametime * ANIMATION_SPEED));
	}
	if (flPlaceNameOpacity)
	{
...
PointWorldText::PointWorldText(const char* pszText, Vector pos)
{
	PrecacheMaterial( TEXT_MATERIAL );
...
int PointWorldText::DrawModel( )
{
	...
	IMaterial* pDebugText = g_PlaceName->GetFont();
	if ( !pDebugText )
		return 0;
	...
	color.a *= flPlaceNameOpacity;
	const float distanceSquared = MainViewOrigin().DistToSqr(GetAbsOrigin());
	if (distanceSquared > placeNamesRadiusSquared)
	{
		color.a *= 1.f - min(1.f, ((distanceSquared - placeNamesRadiusSquared) / placeNamesRadiusSquared));
	}
